...
let () =
  ...
  (* Test de la fonction ford_fulkerson*)
    let final_graph_double = ford_fulkerson int_graph _source _sink in
    Printf.printf "graphe double fait " ;
    let final_graph_simple = to_flow_form int_graph final_graph_double in
    Printf.printf "graphe simple fait " ;
    let flot_max = flot_max int_graph final_graph_double _sink in
    let flot_min = flot_min int_graph final_graph_double _source in
    let s = (st_coupe final_graph_double _source) in
    let t = (t_coupe int_graph s) in
  Printf.printf "\n- Test de la fonction ford_fulkerson : id graph -> id -> id -> id graph 
  \n(source : %d et puit : %d) 
  \n-> : flot_max = %d
  \n-> : flot_min (ST-coupe, S = %s et T = %s) = %d\n\n"  _source _sink flot_max (string_of_list_of_nodes s) (string_of_list_of_nodes t) flot_min;

 

  let string_graph_simple = gmap final_graph_simple (string_of_flow_capacity) in
 (*let string_graph_double = gmap final_graph_double (string_of_int) in*)
  (* Draw the graph that has been changed. *)
  let () = export_dot outfile string_graph_simple in
 ...
 (* Test de la fonction find_min_lbl
  ...
  Printf.printf "\n- Test de la fonction apply_changes : id graph -> id arc list -> id graph (modifier le graphe résiduel en foncion d'un chemin trouvé)\n
 ...
 \nRésultat appliqué au graphe 1 (trouver chemin entre 0 et 4) \n-> doit changer/créer les arcs : \ne 0 2 _ 8 -> e 0 2 _ 0\ne 2 4 _ 12 -> e 2 4 _ 4 \nnouveaux arcs : e 2 0 _ 8 et e 4 2 _ 8\n
  : *voir graphe ci-dessous\n\n" ;
  let new_graph = algo int_graph in*)
 ...
 \nRésultat appliqué au graphe 1 (trouver chemin entre 0 et 4) \n-> doit renvoyer la liste des sommets du chemin : [0;2;4] \n
